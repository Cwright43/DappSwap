...
contract AMM {
	...
	function addLiquidity(uint256 _token1Amount, uint256 _token2Amount) external {
		...
		// If-Else for IF this is the FIRST time the function is being called
		// This will only happen if the total shares is ZERO
		if (totalShares == 0) {
			share = 100 * PRECISION;
		} else {
			...
			"must provide equal token amount"
			);
			share = share1;
		}
	...
	// Determine how many token2 tokens must be eposited when depositing liquidty for token1 BITCH
	function calculateToken2Deposit(uint256 _token1Amount) public view returns(uint256 token2Amount) {
		token2Amount = (token2Balance * _token1Amount) / token1Balance;
	}	

	// Determine how many token1 tokens must be eposited when depositing liquidty for token2 BITCH
	...
	function calculateToken1Swap(uint256 _token1Amount) public view returns (uint256 token2Amount) {
		...
		// Calculate Token 2 amount that we are going to get
		
		uint256 token1After = token1Balance + _token1Amount;
		uint256 token2After = K / token1After;
		token2Amount = token2Balance - token2After;

		// Don't let it go! Hold on, please!
		if(token2Amount == token2Balance) {
			token2Amount --;
		}

		// Cannot swap too many tokens - MISS
		require(token2Amount < token2Balance, "swap cannot exceed pool balance U stupid");
	...
	function swapToken1(uint256 _token1Amount) external returns(uint256 token2Amount) {
		...
		// Do swap
		// 1. Transfer token1 out of user wallet to contract
	...
	// Returns amount of token1 received when swapping token 2
	function calculateToken2Swap(uint256 _token2Amount) public view returns (uint256 token1Amount) {
		...
		// Cannot swap too many tokens - MISS
		require(token1Amount < token1Balance, "swap cannot exceed pool balance U stupid");
	...
	function removeLiquidity(uint256 _share) external returns (uint256 token1Amount, uint256 token2Amount) {
		...
		require(
			...
			"cannot withdraw more shares than you have BITCH"
		);
